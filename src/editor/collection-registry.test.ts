...
test("collection dispatch resolves distinct collection adapters", () => {
  ...
  const about = getEditorCollectionAdapter("about");
  ...
  assert.equal(about?.canCreate, false);
  assert.equal(artists?.canCreate, true);
  assert.equal(journal?.canCreate, true);
  assert.equal(works?.canCreate, true);
  assert.equal(news?.canCreate, true);
