...
test("Artists Hero admission derives a slug filename and validates MIME/bytes", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ArtistsHeroAssetError &&
      error.code === "asset-type-mismatch",
  );
  await assert.rejects(
    inspectArtistsHeroCandidate({
      contentId: "../unsafe",
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof ArtistsHeroAssetError &&
      error.code === "asset-decode-failed",
  );
});
...
test("temporary tokens are owner-bound and expiring", async () => {
  ...
  now = 10;
  await assert.rejects(
    store.read(item.token, "test-artist", "workspace-1"),
    (error: unknown) =>
      error instanceof ArtistsHeroAssetError &&
...
test("upload reuses identical target and stages different bytes without mutation", async () => {
  ...
  try {
    const request = (bytes: Uint8Array) => {
      ...
      form.set(
        "file",
        new File([part], "ignored-name.png", { type: "image/png" }),
      );
    ...
    const reuse = await handleArtistsHeroUpload("test-artist", request(png), {
      root: value.assetRoot,
      store: value.store,
      contentExists: () => true,
    });
    assert.equal((await reuse.json()).state, "reuse");
    const collision = await handleArtistsHeroUpload(
      "test-artist",
      request(otherPng),
...
test("multipart upload rejects multiple files and an oversized declared body", async () => {
  ...
  try {
    ...
    const multiple = await handleArtistsHeroUpload(
      "test-artist",
      new Request("http://editor/upload", { method: "POST", body: form }),
      { root: value.assetRoot, store: value.store, contentExists: () => true },
    );
    assert.equal(multiple.status, 400);
    const oversized = await handleArtistsHeroUpload(
      "test-artist",
      new Request("http://editor/upload", {
        method: "POST",
        headers: {
          "content-type": "multipart/form-data; boundary=x",
          "content-length": String(21 * 1024 * 1024 + 1024 * 1024),
        },
        body: "--x--",
      }),
      { root: value.assetRoot, store: value.store, contentExists: () => true },
...
test("legacy Artists .png URLs retain their WebP bytes without retroactive extension validation", async () => {
  const expected = {
    "alana-wilson.png":
      "c0cd658dd22c7f497463736eee1a07f2de44edf6674ceaf3942522078c889163",
    "keisuke-matsuda.png":
      "2cf0114f2f9e5791fb9c17511d75af2e096bbee78aede18977646a6a306cb122",
    "takeyoshi-mitsui.png":
      "d68334ad732e0efdbe72bcef410c61625fc1171875ffe074a07e32015d251599",
  } as const;
  for (const [filename, digest] of Object.entries(expected)) {
    const bytes = await readFile(
      path.resolve("public/images/artists", filename),
    );
  ...
  try {
    ...
    const saved = await saveArtistsEditorDraftWithHero(
      ...
      baseline,
      {
        ...
        workspaceId: "workspace-1",
        proposedSrc: metadata.proposedSrc,
    ...
    const evidence = await new HeroAssetPublishEvidenceStore(value.root).read(
      "artists",
      "test-artist",
    );
    assert.equal(evidence?.operation, "hero-asset-save");
    assert.equal(evidence?.assets[0].sha256, sha(otherPng));
    await assert.rejects(
      value.store.read(metadata.token, "test-artist", "workspace-1"),
    );
  } finally {
    await rm(value.root, { recursive: true });
  }
...
test("content-only Save refreshes pending content evidence and retains its Hero identity", async () => {
  const value = await fixture();
  try {
    const baseline = createArtistsEditorDraft(
      await readArtistsEditorEntry("test-artist", value.contentRoot),
    ...
    const first = await saveArtistsEditorDraftWithHero(
      baseline,
      baseline,
      {
        kind: "temporary",
    ...
    const before = (await new HeroAssetPublishEvidenceStore(value.root).read(
      "artists",
      "test-artist",
    ))!;
    const edited = structuredClone(first);
    edited.data.name = "Later content edit";
    await saveArtistsEditorDraftWithHero(
      edited,
      first,
      { kind: "existing", src: edited.data.hero.image },
      {
        root: value.contentRoot,
        assetRoot: value.assetRoot,
        store: value.store,
    ...
    const after = (await new HeroAssetPublishEvidenceStore(value.root).read(
      "artists",
      "test-artist",
    ))!;
...
test("format change creates the decoded-format URL and preserves the old asset", async () => {
  ...
  try {
    await fs.rename(
      ...
      path.join(value.assetRoot, "legacy.jpg"),
    );
    await writeFile(
      path.join(value.contentRoot, "test-artist/index.yaml"),
    ...
    assert.deepEqual(
      await readFile(path.join(value.assetRoot, "legacy.jpg")),
      png,
    );
  } finally {
    await rm(value.root, { recursive: true });
...
test("content install failure restores existing Hero bytes", async () => {
  ...
  try {
    ...
    const draft = structuredClone(baseline);
    draft.data.name = "Must Roll Back";
    let failed = false;
    const fileSystem = {
      ...
      rename: async (...args: Parameters<typeof fs.rename>) => {
        ...
        if (
          !failed &&
          String(from).includes("-stage/index.yaml") &&
          String(to).endsWith("/index.yaml")
        ) {
          failed = true;
    ...
    await assert.rejects(
      saveArtistsEditorDraftWithHero(
        draft,
        baseline,
        {
          kind: "temporary",
...
test("Publish evidence failure rolls Artists Hero content and asset back and retains the token", async () => {
  const value = await fixture();
  try {
    const baseline = createArtistsEditorDraft(
      await readArtistsEditorEntry("test-artist", value.contentRoot),
    ...
    const draft = structuredClone(baseline);
    draft.data.name = "Must Roll Back";
    const evidenceStore = new HeroAssetPublishEvidenceStore(value.root);
    evidenceStore.write = async () => {
    ...
    assert.equal(
      (await value.store.read(metadata.token, "test-artist", "workspace-1"))
        .metadata.sha256,
      metadata.sha256,
    );
  } finally {
...
test("Artists Hero UI exposes thumbnail, path, drop, select, replace, and remove controls", async () => {
  const component = await readFile(
    path.resolve("src/components/editor/ArtistsDraftForm.astro"),
    "utf8",
  );
  for (const marker of [
    "data-artists-hero-thumbnail",
    "data-artists-hero-canonical-path",
    "data-artists-hero-drop",
    "data-artists-hero-select",
    "data-artists-hero-replace",
    "data-artists-hero-remove",
  ])
    assert.match(component, new RegExp(marker));
  const workspace = await readFile(
    path.resolve("src/pages/editor/artists/workspace/[contentId].astro"),
    "utf8",
  );
...
test("Create Publish evidence failure rolls new content and asset back and retains the token", async () => {
  const value = await fixture();
  try {
    const source = createArtistsEditorDraft(
      await readArtistsEditorEntry("test-artist", value.contentRoot),
    )!;
    const workspace = "evidence-failure-owner";
    const metadata = await value.store.register({
      contentId: `create-${workspace}`,
      workspaceId: workspace,
      originalFilename: "x.png",
    ...
    draft.contentId = "evidence-failure-artist";
    draft.data.hero.image = "/images/artists/evidence-failure-artist.png";
    const evidenceStore = new HeroAssetPublishEvidenceStore(value.root);
    evidenceStore.write = async () => {
      throw new Error("injected evidence failure");
    };
    ...
    assert.equal(
      await fs
        .lstat(path.join(value.contentRoot, draft.contentId))
        .catch(() => undefined),
      undefined,
    );
    assert.equal(
      await fs
        .lstat(path.join(value.assetRoot, "evidence-failure-artist.png"))
        .catch(() => undefined),
      undefined,
    );
    assert.equal(
