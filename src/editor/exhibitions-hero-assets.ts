...
export const EXHIBITIONS_HERO_PREFIX = "/images/exhibitions/";
...
export type ExhibitionsHeroAssetDraft =
...
export type ExhibitionsHeroAssetMetadata = {
  token: string;
  contentId: string;
  workspaceId: string;
...
export class ExhibitionsHeroAssetError extends Error {
  readonly code:
  ...
  constructor(
    ...
    super(message);
    this.name = "ExhibitionsHeroAssetError";
    this.code = code;
  }
}
...
export async function inspectExhibitionsHeroCandidate(input: {
  contentId: string;
  declaredMime: string;
  bytes: Uint8Array;
}) {
  if (!isContentId(input.contentId))
    throw new ExhibitionsHeroAssetError("Invalid Exhibition Content ID", "asset-invalid-request");
  if (!input.bytes.byteLength)
    throw new ExhibitionsHeroAssetError("Image bytes are empty or malformed", "asset-decode-failed");
  if (input.bytes.byteLength > WORKS_ASSET_POLICY.maxBytes)
    throw new ExhibitionsHeroAssetError("Image exceeds the 20 MiB limit", "asset-too-large");
  let media;
  try {
  ...
  if (media.animated || media.frameCount !== 1)
    throw new ExhibitionsHeroAssetError("Animated images are not supported", "asset-unsupported-format");
  if (
    ...
    throw new ExhibitionsHeroAssetError("Image dimensions exceed the supported limit", "asset-too-large");
  if (input.declaredMime !== media.mime)
    throw new ExhibitionsHeroAssetError("Declared MIME does not match decoded image bytes", "asset-type-mismatch");
  const proposedSrc = `${EXHIBITIONS_HERO_PREFIX}${input.contentId}.${media.format}`;
  return {
    proposedSrc,
...
export class TemporaryExhibitionsHeroAssetStore {
  private records = new Map<string, { metadata: ExhibitionsHeroAssetMetadata; file: string }>();
  private root: string;
  ...
  static async create(options: { parentDirectory?: string; ttlMs?: number; now?: () => number } = {}) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink())
      throw new ExhibitionsHeroAssetError("Temporary asset parent is unsafe", "asset-temp-unsafe");
    const ttlMs = options.ttlMs ?? 10 * 60_000;
    const now = options.now ?? Date.now;
    for (const entry of await fs.readdir(parent, { withFileTypes: true })) {
      if (!entry.name.startsWith("kiki-exhibitions-hero-") || !entry.isDirectory() || entry.isSymbolicLink()) continue;
    ...
    const root = await fs.mkdtemp(path.join(parent, "kiki-exhibitions-hero-"));
    await fs.chmod(root, 0o700);
    return new TemporaryExhibitionsHeroAssetStore(await fs.realpath(root), ttlMs, now);
  }
  ...
  async register(input: {
    ...
    if (!OWNER.test(input.workspaceId))
      throw new ExhibitionsHeroAssetError("Invalid workspace", "asset-invalid-request");
    const admitted = await inspectExhibitionsHeroCandidate(input);
    ...
    const metadata: ExhibitionsHeroAssetMetadata = {
      token,
  ...
  private async owned(token: string, contentId: string, workspaceId: string) {
    if (!TOKEN.test(token) || !OWNER.test(contentId) || !OWNER.test(workspaceId))
      throw new ExhibitionsHeroAssetError("Temporary image not found", "asset-temp-not-found");
    const record = this.records.get(token);
    ...
    if (record.metadata.expiresAt <= this.now()) {
      this.records.delete(token);
      await fs.rm(record.file, { force: true });
      throw new ExhibitionsHeroAssetError("Temporary image expired", "asset-temp-expired");
    }
    return record;
  }
  ...
  async read(token: string, contentId: string, workspaceId: string) {
    ...
    if (!stat.isFile() || stat.isSymbolicLink())
      throw new ExhibitionsHeroAssetError("Temporary image is unsafe", "asset-temp-unsafe");
    ...
    try {
      ...
      if (bytes.byteLength !== record.metadata.byteSize || digest(bytes) !== record.metadata.sha256)
        throw new ExhibitionsHeroAssetError("Temporary image changed", "asset-temp-unsafe");
...
export const temporaryExhibitionsHeroAssetStore = TemporaryExhibitionsHeroAssetStore.create();

export function temporaryExhibitionsHeroPreviewUrl(contentId: string, workspaceId: string, token: string) {
  return `/editor/api/exhibitions-hero-preview/${encodeURIComponent(contentId)}/${encodeURIComponent(workspaceId)}/${encodeURIComponent(token)}`;
}
