...
import {
  serializeArtistsEditorDraft,
  type ArtistsSerializedFiles,
} from "./artists-serializer.ts";
...
import {
  artistsContentEvidence,
  createArtistsHeroPublishEvidence,
...
async function directoryFor(
  ...
  if (
    ...
    throw new ArtistsSaveError("Unsafe Artist unit", "invalid-content-id");
  return directory;
}

export async function writeArtistsSerializedFiles(
  contentId: string,
  next: ArtistsSerializedFiles,
  baseline: ArtistsSerializedFiles,
  root = canonicalRoot,
  ...
  try {
    ...
    for (const name of files) {
      ...
      if (!stat.isFile() || stat.isSymbolicLink())
        throw new Error(`Unsafe Artist source: ${name}`);
    ...
    if (failures.length) {
      ...
      throw new ArtistsSaveError(
        "Failed to roll back Artist Save",
        "artists-save-rollback-failed",
...
export async function saveArtistsEditorDraft(
  ...
  await writeArtistsSerializedFiles(
    draft.contentId,
    serializeArtistsEditorDraft(draft),
    entry.canonicalFiles,
    root,
    fileSystem,
  );
...
export async function saveArtistsEditorDraftWithHero(
  ...
  if (hero.kind !== "temporary") {
    ...
    const saved = await saveArtistsEditorDraft(
      draft,
      baseline,
      options.root,
      options.fileSystem,
    ...
    try {
      ...
      try {
        await writeArtistsSerializedFiles(
          draft.contentId,
          serializeArtistsEditorDraft(baseline),
          serializeArtistsEditorDraft(saved),
          options.root,
          options.fileSystem,
        );
        ...
        throw new ArtistsSaveError(
          "Failed to roll back Artist Save after Publish evidence failure",
          "artists-save-rollback-failed",
  ...
  if (
    ...
    draft.data.hero.image !== hero.proposedSrc
  )
    throw new ArtistsSaveError(
      "Artist draft has invalid Hero state",
      "invalid-draft",
    );
  if (draft.contentId !== baseline.contentId)
    throw new ArtistsSaveError(
  ...
  const previousEvidence = await evidenceStore.read("artists", draft.contentId);
  const entry = await readArtistsEditorEntry(draft.contentId, root);
  const canonical = createArtistsEditorDraft(entry);
  if (
    !canonical ||
  ...
  try {
    ...
    for (const name of files) {
      ...
      if (current !== entry.canonicalFiles[name])
        throw new ArtistsSaveError(
          "Canonical Artist changed during Save",
          "canonical-mismatch",
        );
    ...
    await inspectArtistsHeroCandidate({
      contentId: draft.contentId,
      declaredMime: temporary.metadata.mime,
      bytes: stagedBytes,
    });
    ...
    for (const name of files)
      if (
        ...
        throw new Error("Installed Artist content verification failed");
    const evidence = await createArtistsHeroPublishEvidence({
      repositoryRoot,
      contentId: draft.contentId,
        ...
        .catch((failure) => failures.push(failure));
    {
      try {
        ...
        else await evidenceStore.delete("artists", draft.contentId);
      } catch (failure) {
        failures.push(failure);
      }
    }
    if (failures.length) {
      ...
      throw new ArtistsSaveError(
        "Failed to roll back Artist Hero Save",
        "artists-save-rollback-failed",
        { cause: new AggregateError([error, ...failures]) },
      );
    }
