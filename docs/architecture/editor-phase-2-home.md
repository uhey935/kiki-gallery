...
- Production locates `artists` and `about` by ID and throws if either is absent. It renders Artists `square` and About `landscape`; array order does not control current display order. All variants remain canonical data.
- Section href accepts URL or absolute internal path. No cross-collection route abstraction is introduced.
- Home owns Markdown asset references, not files in `public`. Existing references include paths whose files are absent. Upload, replacement, existence enforcement, deletion, and orphan cleanup remain out of scope.
...
The schema requires exactly one `artists` and one `about` section and all nested variants. This formalizes the runtime consumer requirement while retaining flexible hrefs and existing references.
